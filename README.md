...
- 시간에 따라 변하는 데이터 처리
    ...
    - 이러한 변동 가능성이 있는 데이터를 어떻게 처리해야 할지에 대한 기준이 부족해, 해당 컬럼들을 분석 대상에서 제외하고 프로젝트를 진행하였습니다.
    - 이후 관련된 내용을 검색해 본 결과, 시간에 따라 값이 변하는 차원 데이터는 SCD(Slowly Changing Dimension) 방식으로 관리하며, 분석 목적에 따라 Type 1, Type 2 등 적절한 SCD 유형을 적용해야 한다는 것을 알게 되었습니다.
    - 추후에는 **`거주시도명`** 과 **`직장시도명`** 컬럼을 포함한 차원 테이블을 SCD 방식으로 수정하여 대시보드에 반영할 예정입니다.
